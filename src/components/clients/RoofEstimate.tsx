...
export const RoofEstimate: React.FC<RoofEstimateProps> = ({
    ...
    const loadEstimateData = async () => {
      try {
        ...
        if (estimateDoc.exists()) {
          ...
          // Сохраняем все данные, кроме гвоздей 70
          const nails70Index = data.items.findIndex(item => 
            item.name === 'Гвозди 70 (Для монтажа обрешетки)'
          );
          ...
          if (nails70Index !== -1) {
            setEstimateData(prev => ({
              ...data,
              items: [
                ...data.items.slice(0, nails70Index),
                prev.items[nails70Index], // Оставляем текущее значение гвоздей
                ...data.items.slice(nails70Index + 1)
              ]
            }));
          } else {
  ...
  const handleUpdateItem = (index: number, field: keyof typeof estimateData.items[0], value: number) => {
    // Запрещаем изменение количества гвоздей 70
    const isNails70Item = estimateData.items[index].name === 'Гвозди 70 (Для монтажа обрешетки)';
    if (isNails70Item && field === 'quantity') return;
